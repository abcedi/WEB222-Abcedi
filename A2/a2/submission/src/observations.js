...
function speciesCoordinates(data) {
  ...
  for (i = 0; i < data.results.length; i++) {
    console.log(
      `"${data.results[i].species_guess}" observed at coordinates (${data.results[i].location})`
    );
  }
}
...
function speciesCoordinates3(data) {
  for (const observation of data.results) {
    console.log(
      `"${observation.species_guess}" observed at coordinates (${observation.location.replace(
        /,/,
        ', '
      )})`
    );
...
function observationsByQualityGrade(data, qualityGrade) {
  ...
  if (qualityGrade === null) {
    ...
    if (!['research', 'needs_id', 'casual'].includes(grade)) {
      throw new Error('Invalid');
    }

    data.results.forEach((observation) => {
      if (observation.quality_grade === grade) {
        newObservations.push(observation);
      }
    });
  }
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  qualityGrades.forEach((grade) => {
    newObservations = newObservations.concat(observationsByQualityGrade(data, grade));
  });
  return newObservations;
}
...
function transformObservations(data) {
  ...
  data.results.forEach((observation) => {
    transformedResults.push(transformObservation(observation));
  });
  return transformedResults;
}
...
function getObservationsByTaxa(data, ...taxaNames) {
  ...
  taxaNames.forEach((taxon) => {
    if (['animalia', 'aves', 'insecta', 'plantae'].includes(taxon.toLowerCase())) {
      observations = observations.concat(
        data.results.filter(
          (observation) => observation.taxon.iconic_taxon_name.toLowerCase() === taxon.toLowerCase()
        )
      );
    }
  });
...
function getObservationsByLocation(data, options = {}) {
  ...
  return observations.filter((observation) => {
    ...
    if (options.lat.min) {
      const isLatInRange = latitude >= options.lat.min && latitude <= options.lat.max;
      const isLngInRange = longitude >= options.lng.min && longitude <= options.lng.max;
      if (isLatInRange && isLngInRange) {
        return true;
      }
    }

    const isLatMatch = options.lat === latitude;
    const isLngMatch = options.lng === longitude;

    if (isLatMatch && isLngMatch) {
      return true;
    }
...
// i cant figure out how to make the test(lat and lng values return an Array of expected results) pass
...
function getPlaceURLs(data) {
  let newURLs = [];
  data.results.forEach((observation) => {
    observation.place_ids.forEach((place_ids) => {
      newURLs.push(`https://www.inaturalist.org/observations?place_id=${place_ids}`);
    });
  });
  return newURLs;
}
...
function getSpeciesObservations(data) {
  let count = {};
  data.results.forEach((observation) => {
    if (observation.taxon.name in count) {
      count[observation.taxon.name]++;
    } else {
      count[observation.taxon.name] = 1;
    }
  });
  return count;
}
...
function extractSpeciesNames(data) {
  let species = [];
  data.results.forEach((observation) => {
    if (!species.includes(observation.species_guess)) {
      species.push(observation.species_guess);
    }
  });
  return species;
}
